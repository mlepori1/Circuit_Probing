...
def plot_zoomed_in(df, size, gender, figtitle, layer):
    ...
    df_iid = pd.DataFrame.from_dict(
        {
            "Accuracy": [
                df[f" ablated acc {gender} IID"].values[0],
                df[f"random ablate acc mean {gender} IID"].values[0],
                df[f" vanilla acc {gender} IID"].values[0],
            ],
            "Dataset": ["IID", "IID", "IID"],
    ...
    df_gen = pd.DataFrame.from_dict(
        {
            "Accuracy": [
                df[f" ablated acc {gender} Gen"].values[0],
                df[f"random ablate acc mean {gender} Gen"].values[0],
                df[f" vanilla acc {gender} Gen"].values[0],
            ],
            "Dataset": ["OOD", "OOD", "OOD"],
    ...
    with sns.axes_style("darkgrid"):
        g = sns.catplot(
            data=df, kind="bar", x="Dataset", y="Accuracy", hue="Condition"
        ).set(title=figtitle, ylim=(0.5, 1.0))
    ...
    plt.savefig(
        f"./Reflexive_An/{size}_{gender}_reflexives.pdf",
        format="pdf",
        bbox_inches="tight",
    )
...
def plot_everything(df, gender, figtitle, filetitle):
    ...
    for i in range(0, len(plot_df), 12):
        ...
        plt.savefig(
            f"./Reflexive_An/{filetitle}_{str(i)}.pdf",
            format="pdf",
            bbox_inches="tight",
        )
