...
def plot_zoomed_in(df, size, layer):
    ...
    df_iid = pd.DataFrame.from_dict(
        {
            "Accuracy": [
                df[" ablated acc IID"].values[0],
                df["random ablate acc mean IID"].values[0],
    ...
    with sns.axes_style("darkgrid"):
        ...
        ).set(title=f"GPT2-{size} Syntactic Number", ylim=(0.5, 1.0))
    ...
    plt.savefig(
        f"./Syntactic_Number/{size}_syntactic_number.pdf",
        format="pdf",
        bbox_inches="tight",
    )
...
def plot_everything(df, figtitle, filetitle):
    ...
    for i in range(0, len(plot_df), 12):
        ...
        plt.savefig(
            f"./Syntactic_Number/{filetitle}_{str(i)}.pdf",
            format="pdf",
            bbox_inches="tight",
        )
...
def plot_knn(df, figtitle, filetitle):
    ...
    plt.savefig(
        f"./Syntactic_Number/{filetitle}_Syntactic_Number.pdf",
        format="pdf",
        bbox_inches="tight",
    )
