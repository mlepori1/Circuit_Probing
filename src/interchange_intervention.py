...
def get_interchange_model(config):
    # Load in GPT2 Model, convert state dict to HookedTransformer State Dict, Load up weights
...
def residual_stream_patching_hook(
    ...
    # From TransformerLens Demo notebook
...
def evaluate_interchange_intervention(config, model, dataset):
    ...
    for sample in dataset:
        # For each sample, replace the residual stream activation with one generated by the counterfactual data
        # See if this replacement elicits the expected change in model output
        ...
        # As a sanity check, running the model without the hook should provide the correct answer almost every time
        ...
        # Sometimes, the expected answer of the counterfactual and non-counterfactual are the same.
        # Exclude these cases and compute logit difference
    ...
    return counterfactual_correct/total, original_correct/total, logit_diff_mean, len(logit_diffs)/total
    ...
    for target_layer in config["target_layer_list"]:
        for target_position in config["target_position_list"]:
            for residual_location in config["residual_location_list"]:
                ...
                counterfactual_accuracy, original_model_accuracy, logit_diff, conflicting_samples = evaluate_interchange_intervention(config, model, dataloader)
                ...
                output_dict = {
                    ...
                    "% Samples with Diff Counterfactual label": [conflicting_samples]
                }
