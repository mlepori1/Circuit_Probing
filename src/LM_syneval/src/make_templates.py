...
from template.Templates import AgreementTemplate, NPITemplate
from template.Terminals import AgreementTerminals, NPITerminals
...
class MakeNPITemplate:
    ...
    def make_variable_sents(self, preterms, simple=False):
        ...
        p_intrusive = [
            self.terminals[p]
            for p in self.switch_dets(preterms, opt="intrusive" if simple else "")
        ]
        f_intrusive = [
            self.terminals[p]
            for p in self.switch_tense(
                self.switch_dets(preterms, opt="intrusive" if simple else "")
            )
        ]

        p_ungrammatical = [
            self.terminals[p] for p in self.switch_dets(preterms, opt="ungram")
        ]
        f_ungrammatical = [
            self.terminals[p]
            for p in self.switch_tense(self.switch_dets(preterms, opt="ungram"))
        ]
...
class MakeTestCase:
    ...
    def get_rules(self):
        ...
        if templates is not None:
            ...
            sents = self.template.make_variable_sents(
                preterminals, simple=self.test_case.startswith("simple")
            )
            for k in sents.keys():
                if k not in sent_templates:
                    sent_templates[k] = []
        ...
        if len(sent) == 1:
            for wrd in sent[0]:
                ...
                # so we do not generate them!
                    ...
                    wrd not in partial
                    and wrd not in self.template.terminals["D"]
                    and wrd not in self.template.terminals["C"]
                    and not (
                    ...
                    and not (
                        (wrd.startswith("is") and "are " + wrd[3:] in partial)
            ...
            for wrd in sent[0]:
                for x in self.expand_sent(
                    sent=sent[1:], partial=partial + wrd + " ", switch_ds=switch_ds
                ):
                    if x != "None":
                        yield x
