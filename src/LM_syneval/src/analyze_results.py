...
if not args.anim:
    ...
    pickle.dump(
        joined_results,
        open(args.results_file.split(".pickle")[0] + ".joined.pickle", "wb"),
    )
else:
...
def is_more_probable(sent_a, sent_b):
    if len(sent_a) != len(sent_b) and args.unit_type == "word":
        logging.info(
            "ERROR: Mismatch in sentence lengths: (1) ", sent_a, " vs (2) ", sent_b
        )
    ...
    return sum([sent_a[i][1] for i in range(len(sent_a))]) > sum(
        [sent_b[i][1] for i in range(len(sent_b))]
    )
...
def display_agrmt_results(name, sents):
        ...
        if args.mode != "overall":
            ...
            if len(correct_sents[case]) > 0:
                for i in range(len(correct_sents[case][0][0])):
                    if (
                        correct_sents[case][0][0][i][0]
                        == correct_sents[case][0][1][i][0]
                    ):
                ...
                for i in range(len(incorrect_sents[case][0][0])):
                    if (
                        incorrect_sents[case][0][0][i][0]
                        == incorrect_sents[case][0][1][i][0]
                    ):
            ...
            case_accs[case] = float(len(correct_sents[case])) / (
                len(correct_sents[case]) + len(incorrect_sents[case])
            )
    ...
    if args.mode != "overall":
        ...
        case_out.write(
            ...
            + "\n##########\n"
            + "Overall acc: "
            + str(float(overall_correct) / total)
            + "\n"
        )
...
def display_npi_results(name, sents):
    (
        gi_grammatical_sents,
        gi_intrusive_sents,
        iu_intrusive_sents,
        iu_ungrammatical_sents,
        gu_grammatical_sents,
        gu_ungrammatical_sents,
    ) = sents
    overall_gi = 0.0
    overall_iu = 0.0
    overall_gu = 0.0
    ...
    for case in gi_grammatical_sents.keys():
        ...
        if args.mode != "overall":
            if len(gi_grammatical_sents[case]) > 0:
                string = (
                    " ".join([x[0] for x in gi_grammatical_sents[case][0][0]])
                    + " vs. "
                    + " ".join([x[0] for x in gi_grammatical_sents[case][0][1]])
                    + " vs. "
                    + " ".join([x[0] for x in gi_grammatical_sents[case][0][2]])
                )
            else:
                string = (
                    " ".join([x[0] for x in gi_intrusive_sents[case][0][0]])
                    + " vs. "
                    + " ".join([x[0] for x in gi_intrusive_sents[case][0][1]])
                    + " vs. "
                    + " ".join([x[0] for x in gi_intrusive_sents[case][0][2]])
                )
            strings[case] = string
            gi_case_accs[case] = float(len(gi_grammatical_sents[case])) / (
                len(gi_grammatical_sents[case]) + len(gi_intrusive_sents[case])
            )
            iu_case_accs[case] = float(len(iu_intrusive_sents[case])) / (
                len(iu_intrusive_sents[case]) + len(iu_ungrammatical_sents[case])
            )
            gu_case_accs[case] = float(len(gu_grammatical_sents[case])) / (
                len(gu_grammatical_sents[case]) + len(gu_ungrammatical_sents[case])
            )
    ...
    if args.mode != "overall":
        ...
        case_out.write("\n##########\n" + name + "\n##########\n" + "Overall acc:\n")
        case_out.write(
            "OVERALL P(GRAMMATICAL) > P(INTRUSIVE): "
            + str(float(overall_gi) / total_gi)
            + "\n"
        )
        case_out.write(
            "OVERALL P(INTRUSIVE) > P(UNGRAMMATICAL): "
            + str(float(overall_iu) / total_iu)
            + "\n"
        )
        case_out.write(
            "OVERALL P(GRAMMATICAL) > P(UNGRAMMATICAL): "
            + str(float(overall_gu) / total_gu)
            + "\n"
        )
        for case, score in sorted(gi_case_accs.items(), key=operator.itemgetter(1)):
            ...
            case_out.write(
                "Grammatical > ungrammatical: " + str(gu_case_accs[case]) + "\n"
            )
            case_out.write(
                "Intrusive > ungrammatical: " + str(iu_case_accs[case]) + "\n"
            )
        case_out.write("\n")
        case_out.close()
    ...
    if args.mode == "full":
        ...
        fout.write("\n##########\n" + name + "\n##########\n\n")
        fout.write(
            "Examples where the LM prefers the intrusive licensor over the grammatical case:\n"
        )
        for case in gi_intrusive_sents.keys():
            ...
            for c, i, u in gi_intrusive_sents[case]:
                if count < 5:
                    fout.write(
                        "grammatical:" + str(round(sum([x[1] for x in c]), 2)) + "\n"
                    )
                    fout.write("\t".join([x[0] for x in c]) + "\n")
                    fout.write("\t".join([str(round(x[1], 2)) for x in c]) + "\n")
                    fout.write(
                        "intrusive:" + str(round(sum([x[1] for x in i]), 2)) + "\n"
                    )
        ...
        for case in iu_ungrammatical_sents.keys():
            ...
            for c, i, u in iu_ungrammatical_sents[case]:
                if count < 5:
                    ...
                    fout.write("\t".join([str(round(x[1], 2)) for x in i]) + "\n")
                    fout.write(
                        "ungrammatical:" + str(round(sum([x[1] for x in u]), 2)) + "\n"
                    )
        ...
        fout.close()
    return (
        float(overall_gi) / total_gi,
        float(overall_iu) / total_iu,
        float(overall_gu) / total_gu,
    )
