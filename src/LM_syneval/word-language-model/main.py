...
parser = argparse.ArgumentParser(
    description="PyTorch RNN/LSTM language modeling and CCG tagging multitask model"
)
...
parser.add_argument(
    "--ccg_data", type=str, default=None, help="location of the CCG corpus"
)
parser.add_argument(
    "--model",
    type=str,
...
parser.add_argument(
    "--save_lm_data", type=str, default="lm_data.bin", help="path to save the LM data"
)
parser.add_argument("--test", action="store_true", help="test a trained LM")
parser.add_argument(
    "--guess", action="store_true", help="display best guesses at each time step"
)
parser.add_argument(
    "--guessscores", action="store_true", help="display guess scores along with guesses"
)
parser.add_argument(
    "--guessratios",
    action="store_true",
...
parser.add_argument(
    "--guessprobs", action="store_true", help="display guess probs along with guesses"
)
parser.add_argument("--guessn", type=int, default=1, help="output top n guesses")
parser.add_argument(
    "--words",
    action="store_true",
...
parser.add_argument(
    "--trainfname", type=str, default="train.txt", help="name of the training file"
)
parser.add_argument(
    "--validfname", type=str, default="valid.txt", help="name of the validation file"
)
parser.add_argument(
    "--testfname", type=str, default="test.txt", help="name of the test file"
)
...
def get_entropy(o):
    ...
    logprobs = nn.functional.log_softmax(
        o, dim=0
    )  # numerically more stable than two separate operations
            ...
            for g in range(args.guessn):
                if tags:
                    outputguesses.append(
                        corpus.dictionary.idx2tag[int(guesses[corpuspos][g])]
                    )
                else:
                    outputguesses.append(
                        corpus.dictionary.idx2word[int(guesses[corpuspos][g])]
                    )
                if args.guessscores:
                    ##output raw scores
                    outputguesses.append(
                        "{:.3f}".format(float(guessscores[corpuspos][g]))
                    )
                elif args.guessratios:
                    ##output scores (ratio of score(x)/score(best guess)
                    outputguesses.append(
                        "{:.3f}".format(
                            float(guessscores[corpuspos][g])
...
def test_evaluate(
    test_lm_sentences, test_ccg_sentences, lm_data_source, ccg_data_source
):
    ...
    for i in range(len(lm_data_source) + len(ccg_data_source)):
        ...
        data = data.unsqueeze(
            1
        )  # only needed if there is just a single sentence being processed
        ...
        if args.words:
            ...
            if i >= len(lm_data_source):
                get_complexity_apply(
                    output_flat, targets, i - len(lm_data_source), tags=True
                )
            else:
    ...
    for i in range(0, lm_data_source.size(0) + ccg_data_source.size(0) - 1, args.bptt):
        ...
        if i > lm_data_source.size(0):
            data, targets = get_batch(
                ccg_data_source, i - lm_data_source.size(0), evaluation=True
            )
        # LM
        else:
...
def train():
    ...
    order = list(
        enumerate(
            range(0, train_lm_data.size(0) + train_ccg_data.size(0) - 1, args.bptt)
        )
    )
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print(
                ...
                "loss {:5.2f} | ppl {:8.2f}".format(
                    epoch,
                    batch,
                    len(train_lm_data) + len(train_ccg_data) // args.bptt,
                    lr,
...
if not args.test:
    ...
    test_loss = test_evaluate(
        test_lm_sentences, test_ccg_sentences, test_lm_data, test_ccg_data
    )
    print("=" * 89)
    print(
