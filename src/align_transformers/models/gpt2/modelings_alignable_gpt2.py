...
class AlignableGPT2Model(GPT2Model):
    def __init__(self, config, alignment_config=None):
        ...
        if self.alignment_config != None:
            ...
            self.intervention_boundaries = torch.tensor([1.0, 0.0], requires_grad=True)
    ...
    # mlepori update: Made intervention a function, can take in residual states before and after mlp
    def intervene(self, hidden_states, source_hidden_states, batch_size, intervention_token_range, intervention_ids):
        # disjoin
        ...
        for i in range(batch_size):
            if intervention_token_range is not None:
                start = intervention_token_range[0][i]*self.n_embd ### mlepori Edited because of data collation
                end = intervention_token_range[1][i]*self.n_embd
            else:
        ...
        if source_hidden_states != None:
            ...
            boundary_mask = sigmoid_boundary_sigmoid(
                self.intervention_population.repeat(batch_size, 1), 
                0.,
                intervention_boundaries[0] * int(self.searchable_n_embd),
                self.temperature
            )
            # dummy step to be removed.
            boundary_mask = (intervention_ids==0).unsqueeze(dim=-1)*boundary_mask
        ...
        for i in range(batch_size):
            if intervention_token_range is not None:
                start = intervention_token_range[0][i]*self.n_embd # mlepori edit due to data collation
                end = intervention_token_range[1][i]*self.n_embd
            else:
                start = self.alignment_config["token_range"][0]*self.n_embd
                end = self.alignment_config["token_range"][1]*self.n_embd
        ...
        return hidden_states, rotated_hidden_states
    
    # mlepori update: Added function to enable intervtion between attn and mlp
    def block_forward(
        self, block, intervene, location, source_hidden_states,batch_size, intervention_token_range, intervention_ids,  output_rotated_hidden_states_only, hidden_states, layer_past, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, use_cache, output_attentions
    ):
        ...
        if encoder_hidden_states is not None:
            ...
            cross_attn_outputs = block.crossattention(
                hidden_states,
                attention_mask=attention_mask,
                head_mask=head_mask,
        ...
        if intervene and location == "attn":
            hidden_states, rotated_hidden_states = self.intervene(hidden_states, source_hidden_states, batch_size, intervention_token_range, intervention_ids)
            if output_rotated_hidden_states_only:
                if use_cache:
                    outputs = (hidden_states,) + outputs
                else:
                    outputs = (hidden_states,) + outputs[1:]
                return outputs, rotated_hidden_states
        ...
        return outputs, rotated_hidden_states  # hidden_states, present, (attentions, cross_attentions)
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # mlepori edit to decompose block, allowing for interventions
                # between attention and residual stream
                intervene = self.alignment_config != None and i == self.alignment_config["layer"]
                outputs, rotated_hidden_states = self.block_forward(block, intervene, self.alignment_config["location"], source_hidden_states, batch_size, intervention_token_range, intervention_ids, output_rotated_hidden_states_only, hidden_states, layer_past, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, use_cache, output_attentions)
                #outputs = block(
                #    hidden_states,
                #    layer_past=layer_past,
                #    attention_mask=attention_mask,
...
class AlignableGPT2LMHeadModel(GPT2LMHeadModel):
    ...
    def forward(
        ...
        if labels is not None:
            # mlepori edits: took out label shifting
            # Shift so that tokens < n predict n
            #shift_logits = lm_logits[..., :-1, :].contiguous()
            #shift_labels = labels[..., 1:].contiguous()
            # Flatten the tokens
            loss_fct = CrossEntropyLoss()
            #print(torch.sum(torch.argmax(lm_logits[:, -1], dim=-1) == labels[:, -1]))
            loss = loss_fct(lm_logits[:, -1].view(-1, lm_logits.size(-1)), labels[:, -1].view(-1))
