...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        if self.circuit_probing and self.isL0:
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output, attn_update = self.output(self_outputs[0], hidden_states)
...
class BertLayer(nn.Module):
    def __init__(self, config, isL0=False):
        ...
        if config.circuit_probing and self.isL0:
            ...
            if config.probe_component == "mlp":
                attn_l0 = False
                mlp_l0 = True  
            elif config.probe_component == "attention":
                attn_l0 = True
                mlp_l0 = False  
        else:
            attn_l0 = self.isL0
        ...
        self.attention = BertAttention(config, isL0=attn_l0)
        self.is_decoder = config.is_decoder
        ...
        self.intermediate = BertIntermediate(config, isL0=mlp_l0)
        self.output = BertOutput(config, isL0=mlp_l0)
    ...
    def forward(
        ...
        # If circuit probing on attention, just returns the attention update
        if self.circuit_probing and self.probe_component == "attention":
        ...
        # If circuit probing on mlp, just returns the mlp update
        if self.circuit_probing and self.probe_component == "mlp":
            return layer_output
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        if config.l0:
            self.layerL0 = [i >= config.l0_start and i < config.l0_end for i in range(config.num_hidden_layers)]
        else:
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if config.circuit_probing:
                    raise ValueError("Circuit Probing does not support gradient checkpointing right now")
            ...
            # For circuit probing, cut off inference once the relevant update is made, and return that update
            if self.config.circuit_probing and i == self.config.l0_start:
                circuit_probing_updates = layer_outputs[0]
                return circuit_probing_updates
...
class BertPooler(nn.Module):
    def __init__(self, config):
        ...
        if self.isL0 and config.l0_end == float("inf"):
            linear = functools.partial(L0Linear, l0=True, mask_init_value=config.mask_init_value, ablate_mask=config.ablate_mask) 
        else:
            linear = functools.partial(L0Linear, l0=False)
        self.dense = linear(config.hidden_size, config.hidden_size)
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        elif not hasattr(self.config, "l0_end") or self.config.l0_end == -1: # This masks the pooler too
            self.config.l0_end = float("inf")
        if not hasattr(self.config, "circuit_probing"):
            self.config.circuit_probing = False
        if not hasattr(self.config, "probe_component"):
            self.config.probe_component = None

        if self.config.circuit_probing:
            assert self.config.l0  # Must mask for circuit probing
            assert self.config.l0_start + 1 == self.config.l0_end # Circuit probing operates on individual components
            assert self.config.probe_component == "mlp" or self.config.probe_component == "attention" # Probe either attention or mlp
    ...
    def forward(
        ...
        if config.circuit_probing:
            return encoder_outputs # For circuit probing, encoder outputs is just the update
...
class BertForCircuitProbing(BertPreTrainedModel):
    """
    ...
    def __init__(self, config, tokenizer):
        ...
        if not hasattr(self.config, "l0"):
            self.config.l0 = False
        if not hasattr(self.config, "lamb"):
            self.config.lamb = 0.0
        if not hasattr(self.config, "rm_loss"):
            print("Using Soft NN Loss")
            self.config.rm_loss = "soft_NN"
        if not hasattr(self.config, "probe_component"):
            print("Setting probe component to MLP")
            self.config.probe_component = "mlp"

        self.config.circuit_probing = True

        self.isL0 = self.config.l0
        self.lamb = self.config.lamb
        self.temp = 1.

        self.bert = BertModel(config)
        print(self.bert.config.num_hidden_layers)
    ...
    def compute_l0(self):
        ...
        masks = []
        if not self.isL0:
            raise ValueError("Can't compute L0 for a model without masks")
        for layer in self.modules():
    ...
    def forward(
        ...
        # This is a batch of additions to the residual stream from self.config.probe_component
        # Shape should be (batch size x seq_len x hidden dim)
        outputs = self.bert(
            input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            ...
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
            output_attentions=None, # ignored when doing circuit probing
            output_hidden_states=None, # ignored when doing circuit probing
            return_dict=None, # ignored when doing circuit probing
        )
        ...
        outputs = outputs.reshape(-1, config.hidden_size)
